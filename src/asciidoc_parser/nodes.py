...
class Node:
    ...
    def to_dict(self) -> dict:
        ...
        for attr in getattr(self, '_SERIALIZABLE_ATTRS', []):
            if hasattr(self, attr):
                value = getattr(self, attr)
                if value is not None:
                    data[attr] = value

        for attr in getattr(self, '_NODE_ATTRS', []):
            if hasattr(self, attr):
...
class Header(Node):
    ...
    def to_dict(self) -> dict:
        # This override is necessary because the `attributes` dictionary contains
        # lists of nodes, which the base `to_dict` method cannot handle.
        data = {'type': self.node_type.value}
        if self.title:
