...
=== Added
* **Lossless AsciiDoc Serializer & `dumps`/`loads` API (Issues #78, #130)**:
  - Added `serialize_to_asciidoc()`, `dumps()`, and `loads()` supporting round-trip source preservation with exact line ending detection.
* **ASG Schema Alignment for Blocks & Inlines (Issues #105, #106, #109, #128, #129)**:
...
* **Backslash Escaping for Inline Formatting Delimiters (Feature #113, follow-up)**:
  - A backslash preceding a constrained formatting delimiter (e.g. `\*bold*`, `\_italic_`, `\`code\``, `\#marked#`, `\^super^`, `\~sub~`) now suppresses span creation and emits the delimiter characters as plain text with the escape backslash consumed.
  - Unconstrained formatting delimiters (e.g. `\**bold**`, `\__italic__`, `\``code```, `\##marked##`) are likewise escapable with a single leading backslash.
