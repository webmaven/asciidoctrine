...
def get_example_files():
    ...
    if not os.path.exists(examples_dir):
        return []
    return [os.path.join(examples_dir, f) for f in os.listdir(examples_dir) if f.endswith('.adoc')]

@pytest.mark.parametrize("filepath", get_example_files())
def test_example_file_parses(filepath):
    """
    Data-driven test that ensures every .adoc file in the examples directory parses without error.
    """
