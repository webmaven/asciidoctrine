import pytest
import warnings
from asciidoctrine.lark_parser import parse_to_ast, AsciiDocSyntaxError
...
def test_error_in_main_with_includes(test_env, strict):
    ...
    if strict:
        with pytest.raises(AsciiDocSyntaxError) as exc_info:
            parse_to_ast(main_adoc.read_text(encoding="utf-8"), base_dir=str(tmp_path), safe_mode=False, strict=strict)
        ...
        doc = parse_to_ast(main_adoc.read_text(encoding="utf-8"), base_dir=str(tmp_path), safe_mode=False, strict=strict)
...
def test_serialize_without_includes_preprocessor_off(test_env, strict):
    ...
    doc = parse_to_ast(source, base_dir=str(tmp_path), safe_mode=False, strict=strict, preprocess_directives=False)
...
def test_serialize_with_includes_preprocessor_on(test_env, strict):
    ...
    doc = parse_to_ast(source, base_dir=str(tmp_path), safe_mode=False, strict=strict, preprocess_directives=True)
    
    # Serializing should produce a UserWarning about preprocessed includes
    with pytest.warns(UserWarning, match="Original include directives cannot be reconstructed"):
        result = serialize_to_asciidoc(doc)
...
def test_error_in_main_preprocessor_off(test_env, strict):
    ...
    if strict:
        with pytest.raises(AsciiDocSyntaxError) as exc_info:
            parse_to_ast(main_adoc.read_text(encoding="utf-8"), base_dir=str(tmp_path), safe_mode=False, strict=strict, preprocess_directives=False)
        # Even though sub.adoc has an error if expanded, we don't expand it.
        # The error caught is at line 5 of main.adoc
        assert exc_info.value.line in (4, 5)
        assert exc_info.value.filepath in (str(main_adoc), "<root>", None)
    else:
        doc = parse_to_ast(main_adoc.read_text(encoding="utf-8"), base_dir=str(tmp_path), safe_mode=False, strict=strict, preprocess_directives=False)
        assert isinstance(doc, Document)
