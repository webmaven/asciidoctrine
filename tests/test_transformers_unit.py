...
class MultiParagraphExporter(NodeTransformer):
    ...
    def visit_paragraph(self, node: Paragraph, **kwargs):
        if node.inlines and isinstance(node.inlines[0], Text) and "split" in node.inlines[0].value:
...
def test_node_transformer_replace_same_type():
    doc = Document(blocks=[
        Paragraph(inlines=[Text("apple"), Text("cherry")])
    ])
...
def test_node_transformer_replace_different_type():
    doc = Document(blocks=[
        Paragraph(inlines=[Text("please emphasize me")])
    ])
...
def test_node_transformer_expand_node():
    doc = Document(blocks=[
        Paragraph(inlines=[Text("split this block")])
    ])
